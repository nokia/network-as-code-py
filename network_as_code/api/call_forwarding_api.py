...
class CallForwardingAPI:
    ...
    def retrieve_call_forwarding(self, phone_number):
        ...
        response = self.client.post(url="/call-forwardings", json=body)

        error_handler(response)

        return response.json()

    def verify_unconditional_forwarding(self, phone_number: str) -> bool:
        body = {
            "phoneNumber": phone_number
        }
        response = self.client.post(url="/unconditional-call-forwardings", json=body)
